// ==UserScript==
// @name           Youtube_VideoControl
// @namespace      github.com/s-kono
// @description    Youtube VideoControl
// @version        0.20250102.0
// @grant          none
// @match          https://www.youtube.com/watch*
// @run-at         document-idle
// @icon           https://www.youtube.com/s/desktop/435d54f2/img/favicon_144x144.png
// @downloadURL    https://github.com/s-kono/UserScript/raw/main/Youtube_VideoControl.user.js
// @updateURL      https://github.com/s-kono/UserScript/raw/main/Youtube_VideoControl.user.js
// ==/UserScript==

(function() {
    'use strict';

    const css = `
button.Youtube_VideoControl {
  background:transparent; border:none;
  margin: 0 0 0 5px;
  padding: 0;
  & > img {
    border-radius: 12px;
    opacity: 0.8;
  }
}
button.Youtube_VideoControl.rewind {
  margin-left:15px;
}
button.Youtube_VideoControl.min > img {
  border: 3px solid black;
}
    `;
    const style = document.createElement('style');
    style.type = 'text/css';
    style.appendChild(document.createTextNode(css));
    document.head.appendChild(style);

    const def_speed = 2.2;
    const def_gain = 1;
    const print_title = "[Youtube_VideoControl]";

    setTimeout(() => {
        const video = document.querySelector('video');
        const audioCtx = new AudioContext();
        const source = audioCtx.createMediaElementSource(video);
        const gainNode = audioCtx.createGain();
        //gainNode.gain.value = def_gain;
        source.connect(gainNode);
        gainNode.connect(audioCtx.destination);
        video.playbackRate = def_speed;
        let playrate;
        const ctlbar_left = document.querySelector('div.ytp-left-controls');

    // sec
        const btn_rewind10s = document.createElement('button');
        btn_rewind10s.classList.add('Youtube_VideoControl');
        btn_rewind10s.classList.add('rewind');
        btn_rewind10s.classList.add('ten');
        btn_rewind10s.classList.add('sec');
        const img_rewind10s = document.createElement('img');
        img_rewind10s.width = '35';
        img_rewind10s.height = '35';
        img_rewind10s.src = 'data:image/png;base64,';
        btn_rewind10s.appendChild(img_rewind10s);
        btn_rewind10s.addEventListener("click", (e) => {
            playrate = video.playbackRate;
            video.currentTime -= 10;
            setTimeout(() => {
                video.playbackRate = playrate;
                console.log(print_title, "playbackRate:", video.playbackRate);
            }, 1500);
        });
        ctlbar_left.appendChild(btn_rewind10s);

        const btn_forward10s = document.createElement('button');
        btn_forward10s.classList.add('Youtube_VideoControl');
        btn_forward10s.classList.add('forward');
        btn_forward10s.classList.add('ten');
        btn_forward10s.classList.add('sec');
        const img_forward10s = document.createElement('img');
        img_forward10s.width = '35';
        img_forward10s.height = '35';
        img_forward10s.src = 'data:image/png;base64,';
        btn_forward10s.appendChild(img_forward10s);
        btn_forward10s.addEventListener("click", (e) => {
            playrate = video.playbackRate;
            video.currentTime += 10;
            setTimeout(() => {
                video.playbackRate = playrate;
                console.log(print_title, "playbackRate:", video.playbackRate);
            }, 1500);
        });
        ctlbar_left.appendChild(btn_forward10s);

        const btn_rewind30s = document.createElement('button');
        btn_rewind30s.classList.add('Youtube_VideoControl');
        btn_rewind30s.classList.add('rewind');
        btn_rewind30s.classList.add('thirty');
        btn_rewind30s.classList.add('sec');
        const img_rewind30s = document.createElement('img');
        img_rewind30s.width = '35';
        img_rewind30s.height = '35';
        img_rewind30s.src = 'data:image/png;base64,';
        btn_rewind30s.appendChild(img_rewind30s);
        btn_rewind30s.addEventListener("click", (e) => {
            playrate = video.playbackRate;
            video.currentTime -= 30;
            setTimeout(() => {
                video.playbackRate = playrate;
                console.log(print_title, "playbackRate:", video.playbackRate);
            }, 1500);
        });
        ctlbar_left.appendChild(btn_rewind30s);

        const btn_forward30s = document.createElement('button');
        btn_forward30s.classList.add('Youtube_VideoControl');
        btn_forward30s.classList.add('forward');
        btn_forward30s.classList.add('thirty');
        btn_forward30s.classList.add('sec');
        const img_forward30s = document.createElement('img');
        img_forward30s.width = '35';
        img_forward30s.height = '35';
        img_forward30s.src = 'data:image/png;base64,';
        btn_forward30s.appendChild(img_forward30s);
        btn_forward30s.addEventListener("click", (e) => {
            playrate = video.playbackRate;
            video.currentTime += 30;
            setTimeout(() => {
                video.playbackRate = playrate;
                console.log(print_title, "playbackRate:", video.playbackRate);
            }, 1500);
        });
        ctlbar_left.appendChild(btn_forward30s);

    // min
        const btn_rewind10m = document.createElement('button');
        btn_rewind10m.classList.add('Youtube_VideoControl');
        btn_rewind10m.classList.add('rewind');
        btn_rewind10m.classList.add('ten');
        btn_rewind10m.classList.add('min');
        const img_rewind10m = document.createElement('img');
        img_rewind10m.width = '35';
        img_rewind10m.height = '35';
        img_rewind10m.src = 'data:image/png;base64,';
        btn_rewind10m.appendChild(img_rewind10m);
        btn_rewind10m.addEventListener("click", (e) => {
            playrate = video.playbackRate;
            video.currentTime -= 10 * 60;
            setTimeout(() => {
                video.playbackRate = playrate;
                console.log(print_title, "playbackRate:", video.playbackRate);
            }, 1500);
        });
        ctlbar_left.appendChild(btn_rewind10m);

        const btn_forward10m = document.createElement('button');
        btn_forward10m.classList.add('Youtube_VideoControl');
        btn_forward10m.classList.add('forward');
        btn_forward10m.classList.add('ten');
        btn_forward10m.classList.add('min');
        const img_forward10m = document.createElement('img');
        img_forward10m.width = '35';
        img_forward10m.height = '35';
        img_forward10m.src = 'data:image/png;base64,';
        btn_forward10m.appendChild(img_forward10m);
        btn_forward10m.addEventListener("click", (e) => {
            playrate = video.playbackRate;
            video.currentTime += 10 * 60;
            setTimeout(() => {
                video.playbackRate = playrate;
                console.log(print_title, "playbackRate:", video.playbackRate);
            }, 1500);
        });
        ctlbar_left.appendChild(btn_forward10m);

        const btn_rewind30m = document.createElement('button');
        btn_rewind30m.classList.add('Youtube_VideoControl');
        btn_rewind30m.classList.add('rewind');
        btn_rewind30m.classList.add('thirty');
        btn_rewind30m.classList.add('min');
        const img_rewind30m = document.createElement('img');
        img_rewind30m.width = '35';
        img_rewind30m.height = '35';
        img_rewind30m.src = 'data:image/png;base64,';
        btn_rewind30m.appendChild(img_rewind30m);
        btn_rewind30m.addEventListener("click", (e) => {
            playrate = video.playbackRate;
            video.currentTime -= 30 * 60;
            setTimeout(() => {
                video.playbackRate = playrate;
                console.log(print_title, "playbackRate:", video.playbackRate);
            }, 1500);
        });
        ctlbar_left.appendChild(btn_rewind30m);

        const btn_forward30m = document.createElement('button');
        btn_forward30m.classList.add('Youtube_VideoControl');
        btn_forward30m.classList.add('forward');
        btn_forward30m.classList.add('thirty');
        btn_forward30m.classList.add('min');
        const img_forward30m = document.createElement('img');
        img_forward30m.width = '35';
        img_forward30m.height = '35';
        img_forward30m.src = 'data:image/png;base64,';
        btn_forward30m.appendChild(img_forward30m);
        btn_forward30m.addEventListener("click", (e) => {
            playrate = video.playbackRate;
            video.currentTime += 30 * 60;
            setTimeout(() => {
                video.playbackRate = playrate;
                console.log(print_title, "playbackRate:", video.playbackRate);
            }, 1500);
        });
        ctlbar_left.appendChild(btn_forward30m);

        const range = document.createElement('input');
        range.max = 900;
        range.type = 'range';
        range.min = 0;
        range['valueAsNumber'] = 0;
        range.style = 'width:200px; height:6px; margin:auto 10px;';
        range.onchange = range.oninput =e=> {
            gainNode.gain.value = def_gain + (range['valueAsNumber'] / 100);
            console.log(print_title, "gain.value:", gainNode.gain.value);
        };
        ctlbar_left.appendChild(range);

        console.log(print_title);
    }, 2000);
})();

