...
button.${us_name}.rewind {
  margin-left: 10px;
}
button.${us_name}.min > img {
  border: 2px solid red;
}
button.${us_name}:hover {
  opacity: 0.5;
}
    `;
    ...
    function main() {
        if(document.getElementsByClassName(us_name).length > 0) {
            console.log(`[${us_name}] main() configured`);
            if(url != location.href) {
                ...
                if(document.querySelector(tooltip_path) && document.querySelector(tooltip_path).innerText.match(/人が視聴中| watching now/)){
                    console.log(`[${us_name}] now streaming`);
                    video.playbackRate = 1;
                } else {
                    video.playbackRate = def_speed;
                }
                console.log(`[${us_name}] reset playbackRate`);
            }
            url = location.href;
            return;
        }
        console.log(`[${us_name}] main() AAA`);
        ...
        try {
            ...
            if(!tooltip || !tooltip.innerText) {
                console.log(`[${us_name}] tooltip null`);
                setTimeout(() => {
                    console.log(`[${us_name}] call main(): tooltip null`);
                    main();
                }, 6000);
                return;
            }
            if(tooltip.innerText.match(/人が視聴中| watching now/)){
                ...
                console.log(`[${us_name}] set_speed ${def_speed}`);
                video.playbackRate = def_speed;
            }
            let playrate;
        ...
        // min
          ...
          */
            ...
            console.log(`[${us_name}] main() error: ${e}`);
        }

        console.log(`[${us_name}] main() end`);
    }
    ...
    setTimeout(() => {
        const title_observer = new MutationObserver(function(mutations) {
            if(location.href.match(/\/watch/)) {
                setTimeout(() => {
                    console.log(`[${us_name}] main() run from title_observer`);
                    main();
                }, 1500);
            }
        });
        ...
        if(location.href.match(/\/watch/)) {
            console.log(`[${us_name}] main() run init`);
            main();
        }
    }, 2000);
})();
